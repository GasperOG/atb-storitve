...
export default function DodajNajemPage() {
  ...
  const [prefillData, setPrefillData] = useState<Partial<Rental> | undefined>(undefined);

  useEffect(() => {
    // Preberi podatke za predizpolnitev iz sessionStorage (npr. ob ponavljanju arhiviranega najema)
    const stored = sessionStorage.getItem("prefillRental");
    if (stored) {
      try {
        const parsed: Partial<Rental> = JSON.parse(stored);
        // ID se ne prenese — novi najem dobi nov ID
        delete parsed.id;
        setPrefillData(parsed);
      } catch {
        // nič
      }
      sessionStorage.removeItem("prefillRental");
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-300 via-gray-600 to-blue-900 py-10 px-4 text-blue-900">
      <div className="max-w-4xl mx-auto bg-white/90 rounded-2xl shadow-lg p-6 sm:p-8 animate-fade-in">
        ...
        <AddRentalForm kovcki={kovcki} rentals={rentals} setRentals={setRentals} prefillData={prefillData} />
      </div>
      <style jsx global>{`
        @keyframes fade-in {
