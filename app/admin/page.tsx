...
export default function AdminHome() {
  ...
  useEffect(() => {
    ...
    const mName = document.cookie.match('(^|;)\\s*' + 'admin_name' + '\\s*=\\s*([^;]+)');
    const nameVal = mName ? decodeURIComponent(mName[2]) : null;
    const mEmail = document.cookie.match('(^|;)\\s*' + 'admin_email' + '\\s*=\\s*([^;]+)');
    const emailVal = mEmail ? decodeURIComponent(mEmail[2]) : null;
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-gray-300 via-gray-600 to-blue-900 transition-colors duration-500">
      <main className="p-4 sm:p-8 md:p-12 max-w-5xl mx-auto font-sans">
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold mb-8 sm:mb-12 text-center text-gray-900 drop-shadow-md">
          Admin Nadzorna Plošča{adminName ? ` — ${adminName}` : (adminEmail ? ` — ${String(adminEmail).split('@')[0]}` : '')}
        </h1>
        ...
        <div className="flex flex-col sm:flex-row justify-center mt-16 mb-8 gap-4 px-4">
          <Link
            href="/"
            className="w-full sm:w-[220px] flex items-center justify-center text-center bg-white/80 hover:bg-white text-blue-900 hover:text-blue-700 border border-blue-300 rounded-full px-6 py-3 font-semibold shadow-lg transition-all duration-300 text-lg focus:outline-none animate-fade-in-up group hover:scale-105 hover:shadow-2xl hover:bg-gradient-to-br hover:from-blue-100 hover:via-blue-200 hover:to-blue-300 hover:border-blue-500 active:scale-100"
            style={{ textAlign: 'center', animationDelay: `${menuItems.length * 80 + 200}ms`, animationFillMode: 'both' }}
            tabIndex={0}
          >
