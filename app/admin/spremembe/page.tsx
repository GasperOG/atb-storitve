  ...
  const fmt = (t: unknown) => {
    try {
      if (t && typeof (t as { toDate?: unknown }).toDate === "function") {
        ...
        return d ? d.toLocaleString('sl-SI') : "";
      }
      const d = t ? new Date(String(t)) : null;
      return d && !Number.isNaN(d.getTime()) ? d.toLocaleString('sl-SI') : "";
        ...
        <div className="bg-white/95 rounded-2xl shadow-2xl p-6 sm:p-8 animate-fade-in">
          ...
          <div className="space-y-3">
            ...
            {items.map((a) => {
              ...
              // Prefer showing action letter (Slovene): Dodajanje=D, Posodobitev=P, Brisanje=B
              const action = String(a.action ?? '').toLowerCase();
              let initials = '';
              if (action === 'create' || action === 'dodajanje' || action === 'add') initials = 'D';
              else if (action === 'update' || action === 'posodobitev' || action === 'edit' || action === 'modify') initials = 'P';
              else if (action === 'delete' || action === 'brisanje' || action === 'remove') initials = 'B';

              // fallback to actor initials if action not recognized
              if (!initials) {
                initials = 'NA';
