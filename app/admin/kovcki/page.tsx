...
import { updateKovcek, deleteKovcek, pridobiZadnjeSpremembe } from "@/lib/firestore";
...
export default function KovckiPage() {
  ...
  const [lastActions, setLastActions] = useState<Record<string, 'D'|'P'|'I'>>({});
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'audit'), { message: `Dodajanje kovček ${nextId}`, collection: 'kovcki', action: 'create', itemId: nextId, device, user: adminEmail ?? null, timestamp: serverTimestamp() });
      // mark local action as Create (D)
      setLastActions(prev => ({ ...prev, [nextId]: 'D' }));

      // best-effort: verify audit entry exists
      try {
        const recent = await pridobiZadnjeSpremembe(10);
        const found = recent.find((r) => String((r as Record<string, unknown>)['itemId']) === String(nextId) && String((r as Record<string, unknown>)['action']) === 'create');
        console.log('Audit create found:', !!found, found ?? null);
      } catch (err) {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      const adminEmail = getCookie('admin_email') ?? null;
      await updateKovcek(editingId, { name: editName.trim(), desc: editDesc.trim() }, { device, user: adminEmail });
      // mark local action as Update (P)
      setLastActions(prev => ({ ...prev, [editingId]: 'P' }));

      // best-effort: verify audit entry exists for update
      try {
        const recent = await pridobiZadnjeSpremembe(10);
        const found = recent.find((r) => String((r as Record<string, unknown>)['itemId']) === String(editingId) && String((r as Record<string, unknown>)['action']) === 'update');
        console.log('Audit update found:', !!found, found ?? null);
      } catch (err) {
        console.warn('Audit verification failed', err);
      }
    } catch (e) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const adminEmail = getCookie('admin_email') ?? null;
      // mark local action as Delete (I) before deletion
      setLastActions(prev => ({ ...prev, [deleteModal.kovcek!.id]: 'I' }));
      await deleteKovcek(deleteModal.kovcek.id, { device, user: adminEmail });
      // best-effort: verify audit entry exists for delete
      try {
        const recent = await pridobiZadnjeSpremembe(10);
        const found = recent.find((r) => String((r as Record<string, unknown>)['itemId']) === String(deleteModal.kovcek!.id) && String((r as Record<string, unknown>)['action']) === 'delete');
        console.log('Audit delete found:', !!found, found ?? null);
      } catch (err) {
        console.warn('Audit verification failed', err);
      }
    } catch (e) {
      console.error(e);
      alert('Napaka pri brisanju kovčka.');
      return;
    }
    closeDeleteModal();
    fetchKovcki();
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6 animate-fade-in">
            {filteredKovcki.map((k) => (
              <div key={k.id} className="relative bg-gradient-to-br from-gray-300 via-gray-600 to-blue-900 rounded-2xl shadow-lg p-8 flex flex-col justify-between border border-blue-300 text-white transform transition-transform duration-200 hover:-translate-y-1 hover:shadow-2xl">
                {lastActions[k.id] && (
                  <div className="absolute top-3 right-3 w-9 h-9 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center font-bold text-sm text-white">
                    {lastActions[k.id]}
                  </div>
                )}
                <div>
