...
interface RentalsTableProps {
  ...
  showRepeat?: boolean;  // če je true, prikaži gumb Ponovi
}
...
export default function RentalsTable({ rentals, kovcki, showArchive = true, showRepeat = false }: RentalsTableProps) {
  ...
  return (
    <div className="max-w-6xl mx-auto mt-8 overflow-x-auto">
      ...
      <div className="mb-2 flex flex-wrap gap-4 text-sm text-gray-700 items-center">
        ...
        {showRepeat && <span><b>Ponovi</b> – dodaj nov najem s temi podatki</span>}
      ...
      <table className="min-w-full bg-white border border-gray-200 rounded shadow">
        <thead>
          <tr className="bg-gray-100 text-left">
            ...
            {showRepeat && <th className="py-2 px-4 border-b border-gray-300">Ponovi</th>}
        ...
        <tbody>
          {sortedFilteredRentals.length === 0 ? (
            <tr key="empty">
              <td colSpan={(showArchive ? 7 : 6) + (showRepeat ? 1 : 0)} className="text-center py-4 text-gray-500">
                Ni vnosov.
              </td>
            </tr>
          ) : (
            sortedFilteredRentals.map((rental, index) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="py-2 px-4 border-b border-gray-200">{fmtDate(rental.startDate)}</td>
                  <td className="py-2 px-4 border-b border-gray-200">{fmtDate(rental.endDate)}</td>
                  <td className="py-2 px-4 border-b border-gray-200">
                    <button
                  ...
                  {showRepeat && (
                    <td className="py-2 px-4 border-b border-gray-200">
                      <button
                        onClick={() => {
                          sessionStorage.setItem("prefillRental", JSON.stringify(rental));
                          router.push("/admin/dodaj-najem");
                        }}
                        className="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded-full shadow-md transition-all duration-200 transform hover:scale-110"
                      >
                        Ponovi
                      </button>
                    </td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-black font-bold">{fmtDate(selectedRental.startDate)}</span>
                </div>
                <div className="flex justify-between">
                  <strong className="text-gray-700">Konec najema:</strong>
                  <span className="text-black font-bold">{fmtDate(selectedRental.endDate)}</span>
                </div>
                <div className="flex justify-between">
