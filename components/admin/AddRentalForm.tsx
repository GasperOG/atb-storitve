...
type Props = {
  ...
  prefillData?: Partial<Rental>;
};

export default function AddRentalForm({ kovcki, rentals, setRentals, prefillData }: Props) {
  ...
  const [formData, setFormData] = useState<Rental & {
    ...
    itemId: prefillData?.itemId ?? "",
    startDate: prefillData?.startDate ?? "",
    endDate: prefillData?.endDate ?? "",
    hasOwnKovcek: prefillData?.hasOwnKovcek ?? false,
    hasOwnNosilci: prefillData?.hasOwnNosilci ?? false,
    thuleKit: prefillData?.thuleKit ?? "",
    thuleNoge: prefillData?.thuleNoge ?? "",
    thulePalice: prefillData?.thulePalice ?? "",
    keyNumber: prefillData?.keyNumber ?? "",
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData((prev) => {
      ...
      // Preveri, da konec najema ni pred začetkom
      if (name === "endDate" && value && updated.startDate && value < updated.startDate) {
        alert("Datum demontaže ne more biti pred datumom montaže!");
        return { ...prev, endDate: "" };
      }
      if (name === "startDate" && value && updated.endDate && updated.endDate < value) {
        // Resetiraj endDate, ker je postal neveljaven
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (phoneDigits.length < 9) {
      alert("Telefonska številka mora imeti vsaj 9 številk.");
      return;
    }
