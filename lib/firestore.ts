  ...
  try {
    ...
    try {
      for (const key of Object.keys(data)) {
        ...
        const newVal = (data as Record<string, unknown>)[key];
        if (JSON.stringify(oldVal) !== JSON.stringify(newVal)) {
          await addDoc(collection(db, 'audit'), { collection: 'kovcki', action: 'update', itemId: id, field: key, oldValue: oldVal ?? null, newValue: newVal ?? null, device: meta?.device, user: meta?.user ?? null, timestamp: serverTimestamp() });
        }
      }
    } catch { /* ignore */}
  } catch (e) {
